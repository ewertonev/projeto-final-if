async function getQuartosComReservas(data = {}) {
	const filtros = [];
	const valores = [];
	...
	valores.unshift(data.data_base ?? new Date());

	const [rows] = await this.DB.execute(sql, valores);
	return rows;
}
async function getIntervalosDisponiveisAPartirDeHoje(data = {}) {
	const dataBase = data.data_base ?? new Date();

	const rows = await this.getQuartosComReservas({
...
async function getQuartosDisponiveisNoIntervalo(data) {
	const valores = [];
...
function montarQuartosComIntervalos(rows, dataBase) {
...
function mostrarIntervalosValidos(reservas, dataBase = new Date()) {
	const intervalos = [];
